...
**`Tenant.loginEmail` write path:** `PATCH /admin/tenants/:id` (`updateTenant` in `adminTenantController.js`) accepts `loginEmail` in the request body, following the same conditional destructure-into-`data`/`beforeData`/`afterData` pattern as `name`/`rateLimitHours`/`status`. (Note: an earlier version of this codebase shipped without this — `createTenant`/`updateTenant` silently ignored `loginEmail`, so `send-password-reset` always 400'd on a fresh tenant. Caught during doc review and fixed in `fb30e44`.)
